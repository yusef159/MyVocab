...
export function saveLastCompletedSession(session: SavedFlashcardSession): void {
  try {
...
export function getLastCompletedSession(): SavedFlashcardSession | null {
  try {
    ...
    return {
      wordIds: parsed.wordIds,
      currentIndex: 0, // Always start from beginning when regenerating
      filterType: parsed.filterType ?? 'all',
      dateRange: typeof parsed.dateRange === 'number' ? parsed.dateRange : 30,
      savedAt: typeof parsed.savedAt === 'string' ? parsed.savedAt : new Date().toISOString(),
      knownCount: typeof parsed.knownCount === 'number' ? parsed.knownCount : 0,
